...
{ 
  ...
  # source('C:/Users/Administrator/Desktop/Scripts/DnDRuf/dev/v1.1/InitiatSim.R', local = TRUE)
  ...
  # source('C:/Users/tui81100/Dropbox/My PC (UncleSplashysSaddnessEmporium)/Desktop/Scripts/DnDRuf/dev/v1.1/InitiatSim.R')
  source('SupportFunctions/rollr.R', local = TRUE)
  source('InitiatSim.R', local = TRUE)
...
# Function Start -----
  ...
  if (!is.na(Seed)){
    ...
    print(paste0("CombatSim Seed: ", Seed))
  }
  ...
  engage <- function(data = df,
                     summary = Summary,
                     person = k,
                     output)
  { # if the opposing party is not dead, continue.
    if (length(grep(TRUE, SideAlive)) > 1){
      # Randomly select a person on an opposing side who is alive to engage with
      data$EngagedWith[person] <- sample(data$Name[data$Side != data$Side[person] & data$LifeStatus == "Alive"], 1)
      ...
      if (output == "text"){
        ...
        Action <- paste0(data$Name[person], " (", data$Group[person], ")", " has targeted ", data$EngagedWith[person], ".")
                    ...
                    output)
  {
    ...
    if (suicide == TRUE){
      ...
      data$HP[data$Name == data$Name[person]] <- 0
      # Change their health status to unconscious
      data$LifeStatus[data$Name == data$Name[person]] <- "Unconscious"
      # Note which round they went down in
      data$RoundKilled[data$Name == data$Name[person]] <- Round
    }
    # If the person kills someone else
    if (suicide == FALSE){
      # Reduce target's health to 0
      data$HP[data$Name == data$EngagedWith[person]] <- 0
      # Change their life status to unconscious
      data$LifeStatus[data$Name == data$EngagedWith[person]] <- "Unconscious"
      # Note what round they went down in
      data$RoundKilled[data$Name == data$EngagedWith[person]] <- Round
      # Increase their kill count by 1
      data$EnemiesKilled[data$Name == data$Name[person]] <- data$EnemiesKilled[data$Name == data$Name[person]] + 1
    }
    # Routing output based upon text or table, because I can;t do both simultaneously
    if (output == "text"){
  ...
  if (Ending == "Death"){
                ...
                if (!is.na(df$EngagedWith[k]) & df$LifeStatus[df$Name == df$EngagedWith[k]] == "Alive"){
                  ...
                  if (ATK == 1){
                    ...
                    if (df$HP[df$Name == df$Name[k]] <= 0){
                      df <- death(suicide = TRUE,
                                  output = "table")
                      Summary <- death(desc = paste0(df$Name[k], " (", df$Group[k], ")", " has accidentally committed suicide!"), 
                                       suicide = TRUE,
                                       output = "text")
                    }
                  }
                  ...
                  if (ATK == 20){
                    ...
                    if (ATK == 20){
                      ...
                      if (ATK == 20){
                        ...
                        Summary <- death(desc = paste0("In an unparalleled display of brutality and carnage, ", df$Name[k], " (", df$Group[k], ")", " has annihilated ", df$EngagedWith[k]," with a single attack."), 
                                         output = "text")
                      }
                    }
                    ...
                    if (ATK != 20){
                      ...
                      if (df$HP[df$Name == df$EngagedWith[k]] <= 0){
                        # They will kill their opponent in a single swing.
                        df <- death(output = "table")
                        Summary <- death(desc = paste0("After taking a critical hit, ", df$EngagedWith[k], " (", df$Group[df$Name == df$EngagedWith[k]], ")", " has fallen!"), 
                                         output = "text")
                      }
                    }
                  }
                  ...
                  if (ATK > 1 & ATK < 20){
                    ...
                    if (ATK > df$AC[df$Name == df$EngagedWith[k]]){
                      ...
                      if (df$HP[df$Name == df$EngagedWith[k]] <= 0){
                        df <- death(output = "table")
                        Summary <- death(desc = paste0(df$EngagedWith[k], " (", df$Group[df$Name == df$EngagedWith[k]], ")", " has gone down!"), 
                                         output = "text")
                      }
                    }
    ...
    for (i in 1:length(Groups)){
      if (sum(df$LifeStatus != "Alive" & df$Group == Groups[i]) == nPeopleGrp[i]){
        GroupAlive[i] <- FALSE
      }
