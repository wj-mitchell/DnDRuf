...
SurSmiths <- c("Anvill", "Anvilson", "Bellows", "Black", "Blackiron", "Copper", "Coppers", "Farrier", "Fletcher", "Fletchett", "Forger", "Forgeman", "Goldsmith", "Grey", "Greysteel", "Hammer", "Hammett", "Irons", "Yrons", "Ironsmith", "Ironshoe", "Ironhoof", "Kettle", "Kettleblack", "Kettleman", "Potts", "Pott", "Pottaker", "Pound", "Poundstone", "Shields", "Shieldson", "Slagg", "Slagman", "Smith", "Smyth", "Smitts", "Smittens", "Smitty", "Smythett", "Smoke", "Smoker", "Steel", "Steele", "Steelman", "Swords", "Swordson", "Tinn", "Tinman", "Tynn", "Tyne", "Tine")
SurSoldiers <- c("Ackes", "Ax", "Archer", "Bailey", "Banner", "Bannerman", "Bay", "Bayes", "Bones", "Boots", "Bootes", "Bowman", "Chestnut", "Colt", "Colter", "Dice", "Dyce", "Dycen", "Dyson", "Flagg", "Flag", "Helms", "Hightower", "Knight", "Leathers", "Lethers", "March", "Marcher", "Mares", "Mayr", "Mair", "Marks", "Mercer", "Pike", "Pikes", "Pyke", "Pykes", "Pikeman", "Pykeman", "Poleman", "Polman", "Rider", "Ryder", "Shields", "Shieldson", "Slaughter", "Spears", "Speers", "Swords", "Swordson", "Towers", "Wahl", "Wall", "Wahls", "Walls", "Waller")
SurStablers <- c("Bay", "Bayes", "Brand", "Carrier", "Carryer", "Carter", "Carton", "Cartwright", "Chestnut", "Colt", "Colter", "Driver", "Dryver", "Foote", "Handler", "Mares", "Mayr", "Mair", "Porter", "Quicke", "Quick", "Reines", "Reynes", "Reins", "Reyns", "Rider", "Ryder", "Ryde", "Saddler", "Stall", "Stalls", "Staller", "Stallworth", "Stallman", "Swift", "Swyft", "Trainor", "Trainer", "Wain", "Wayne", "Wayn", "Wainwright", "Waynwright")
SurSwampfolk <- c("Banks", "Bankes", "Black", "Blacktide", "Greentide", "Boggs", "Bogg", "Bogs", "Bull", "Buzzfly", "Blackfly", "Shoefly", "Cray", "Craw", "Cricketts", "Crickets", "Darkwater", "Dragonfly", "Dragon", "Eeler", "Ealer", "Eeles", "Eales", "Fisher", "Fishman", "Frogg", "Frogman", "Green", "Greene", "Greenwater", "Blackwater", "Grey", "Gray", "Grove", "Groves", "Hook", "Hooke", "Hopper", "Marsh", "Mayfly", "May", "Moss", "Mosstree", "Greentree", "Poisonweed", "Poisonwood", "Polly", "Pollywog", "Polliwog", "Rafman", "Raftman", "Ratt", "Ratman", "Reed", "Reede", "Reedy", "River", "Rivers", "Rotten", "Rotman", "Scales", "Greenscale", "Blackscale", "Shell", "Shellman", "Sheller", "Shelley", "Skeeter", "Skito", "Small", "Smalls", "Snails", "Snailman", "Stillwater", "Swimmer", "Shwimmer", "Swymmer", "Thick", "Thicke", "Tidewater", "Vines", "Waters", "Watters", "Wurms", "Worms")
SurTownsfolk <- c("Alley", "Allie", "Bailey", "Bell", "Berg", "Berger", "Burg", "Burger", "Brickman", "Brickhouse", "Bridges", "Court", "Gardner", "Gardiner", "Hall", "Heap", "Hightower", "Hood", "House", "Lane", "Lain", "Laine", "Lodge", "Lodges", "Park", "Parks", "Plaza", "Rhoads", "Rhodes", "Roades", "Roof", "Spanner", "Stairs", "Street", "Streets", "Towers", "Towns", "Townsend", "Townes", "Towny", "Towney", "Vista", "Wall", "Wahl", "Woodhouse")
SurWoodsfolk <- c("Ackes", "Ax", "Archer", "Berry", "Biggs", "Bigg", "Birch", "Byrch", "Bird", "Byrd", "Birdett", "Byrdman", "Bloom", "Bowman", "Branch", "Brush", "Buck", "Deere", "Deerman", "Doe", "Feller", "Fletcher", "Flowers", "Forester", "Forrester", "Forrest", "Fox", "Foxx", "Gardner", "Gardener", "Gardiner", "Green", "Greene", "Grove", "Groves", "Harper", "Hatchet", "Hunter", "Huntsman", "Hyde", "Hides", "Hydes", "Jack", "Lodge", "Lodges", "Meadows", "Mole", "Moler", "Moller", "Moss", "Mosstree", "Greentree", "Oaks", "Oakes", "Pine", "Pines", "Pyne", "Pynes", "Sawyer", "Silver", "Silvermoon", "Singer", "Springs", "Springer", "Strider", "Stryder", "Tanner", "Tannerman", "Thick", "Thicke", "Walker", "Woods", "Wood", "Woode", "Wooden", "Woodyn")

## Name dataframe ----
#Concatenating first names and their associated details into a dataframe
rows_first <- c(FirstArchaicFemale, FirstArchaicMale, FirstWestronFemale, FirstWestronMale, FirstNorthishFemale, FirstNorthishMale, FirstSouthronFemale, FirstSouthronMale)
cols_first <- c("Name", "Culture", "Gender")
df_first <- data.frame(matrix(NA, 
                                  nrow = length(rows_first), 
                                  ncol = length(cols_first), 
                                  dimnames = list(1:length(rows_first), cols_first)))
df_first$Name <- rows_first
df_first$Culture <- c(rep("Archaic", (length(FirstArchaicFemale) + length(FirstArchaicMale))),
                          rep("Westron", (length(FirstWestronFemale) + length(FirstWestronMale))),
                          rep("Northish", (length(FirstNorthishFemale) + length(FirstNorthishMale))),
                          rep("Southron", (length(FirstSouthronFemale) + length(FirstSouthronMale))))
df_first$Gender <- c(rep("Female", length(FirstArchaicFemale)), rep("Male", length(FirstArchaicMale)),
                      rep("Female", length(FirstWestronFemale)), rep("Male", length(FirstWestronMale)),
                      rep("Female", length(FirstNorthishFemale)), rep("Male", length(FirstNorthishMale)),
                      rep("Female", length(FirstSouthronFemale)), rep("Male", length(FirstSouthronMale)))

#Concatenating surnames into a dataframe along with their associated details
rows_sur <- c(SurDesertfolk, SurFarmfolk, SurFoodPrep, SurFrozenLands, SurGarmenters, SurIslanders, SurCrafters, 
          SurMountainfolk, SurMerchants, SurMagesScholars, SurRiverfolk, SurSeafolk, SurSmiths, SurSoldiers, 
          SurStablers, SurSwampfolk, SurTownsfolk, SurWoodsfolk)
cols_sur <- c("Name", "Background")
df_sur <- data.frame(matrix(NA, 
                                nrow = length(rows_sur), 
                                ncol = length(cols_sur), 
                                dimnames = list(1:length(rows_sur), cols_sur)))
df_sur$Name <- rows_sur
df_sur$Background <- c(rep("Desertfolk", length(SurDesertfolk)), rep("Farmfolk", length(SurFarmfolk)),
                           rep("Food Makers", length(SurFoodPrep)), rep("Frozenlanders", length(SurFrozenLands)),
                           rep("Garment Makers", length(SurGarmenters)), rep("Islanders", length(SurIslanders)),
                           rep("Masoners, Builders, & Crafters", length(SurCrafters)), rep("Miners & Mountainfolk", length(SurMountainfolk)),
                           rep("Merchants, Jewelers, & Moneylenders", length(SurMerchants)), rep("Priests, Scholars, & Mages", length(SurMagesScholars)),
                           rep("Riverfolk", length(SurRiverfolk)), rep("Seafolk", length(SurSeafolk)),
                           rep("Smiths", length(SurSmiths)), rep("Soldiers, Guards, & Warriors", length(SurSoldiers)),
                           rep("Stable Hands & Drivers", length(SurStablers)), rep("Swampfolk", length(SurSwampfolk)),
                           rep("Townfolk & Cityfolk", length(SurTownsfolk)), rep("Woodsfolk", length(SurWoodsfolk)))

# Creating a new dataframe which will store the first and last names of the number of characters denoted in nPeople, as well as the characters, gender, background, and culture.
cols <- c("First", "Surname", "Gender", "Background", "Culture")
df <- data.frame(matrix(NA, 
                        nrow = nPeople, 
                        ncol = length(cols), 
                        dimnames = list(1:nPeople, cols)))
## Simulation ----

# We are using the sample function to randomly select from the unique values present in the Culture column a number of time equivalent to the number of people we want to simulate, and doing so with replacement, so we could select the same value more than once.
# All probabilities are equal
df$Culture <- sample(unique(df_first$Culture), 
                     nPeople, 
                     replace = TRUE) 

# We are using the sample function to randomly select from the unique values present in the Background column a number of time equivalent to the number of people we want to simulate, and doing so with replacement, so we could select the same value more than once.
# All probabilities are equal
df$Background <- sample(unique(df_sur$Background), 
                        nPeople, 
                        replace = TRUE)

# It may be the case that we want to skew the probability of selecting one gender.
# This if statement should automatically rebalance the probabilities of the other genders by checking which gender no longer meets the default values and shifting the other two, allowing the function to still work fine even when only specifying one gender probability.
# If the probabiltiy of any genders have been altered from the default setting and do not balance...
pNonbinary_output <- pNonbinary
pMale_output <- pMale
pFemale_output <- pFemale

if (pNonbinary + pMale + pFemale != 1){
  # But only nonbinary individuals were altered, adjust the other two accordingly
  if (pNonbinary != 0.01 & pMale == 0.49 & pFemale == 0.50){
    pMale_output <- (1 - pNonbinary) * (pMale/(pMale + pFemale))
    pFemale_output <- (1 - pNonbinary) * (pFemale/(pMale + pFemale))
  }
  # But only males were altered, adjust the other two accordingly
  if (pNonbinary == 0.01 & pMale != 0.49 & pFemale == 0.50){
    pNonbinary_output <- (1 - pMale) * (pNonbinary/(pNonbinary + pFemale))
    pFemale_output <- (1 - pMale) * (pFemale/(pNonbinary + pFemale))
  }
  # But only females was altered, adjust the other two accordingly
  if (pNonbinary == 0.01 & pMale == 0.49 & pFemale != 0.50){
    pNonbinary_output <- (1 - pFemale) * (pNonbinary/(pNonbinary + pMale))
    pMale_output <- (1 - pFemale) * (pMale/(pNonbinary + pMale))
  }
  # But only males and nonbinary individuals were altered, adjust the females accordingly
  if (pNonbinary != 0.01 & pMale != 0.49 & pFemale == 0.50){
    pFemale_output <- (1 - pNonbinary - pMale)
    if (1 - pNonbinary - pMale < 0){
      pNonbinary_output <- pNonbinary/(pNonbinary + pMale + pFemale)
      pFemale_output <- pFemale/(pNonbinary + pMale + pFemale)
      pMale_output <- pMale/(pNonbinary + pMale + pFemale)
    }
  }
  # But only females and nonbinary individuals were altered, adjust the males accordingly
  if (pNonbinary != 0.01 & pMale == 0.49 & pFemale != 0.50){
    pMale_output <- (1 - pNonbinary - pFemale)
    if (1 - pNonbinary - pFemale < 0){
      pNonbinary_output <- pNonbinary/(pNonbinary + pMale + pFemale)
      pFemale_output <- pFemale/(pNonbinary + pMale + pFemale)
      pMale_output <- pMale/(pNonbinary + pMale + pFemale)
    }
  }
  # But only males and females were altered, adjust the nonbinary individuals accordingly
  if (pNonbinary == 0.01 & pMale != 0.49 & pFemale != 0.50){
    pNonbinary_output <- (1 - pFemale - pMale)
    if (1 - pFemale - pMale < 0){
      pNonbinary_output <- pNonbinary/(pNonbinary + pMale + pFemale)
      pFemale_output <- pFemale/(pNonbinary + pMale + pFemale)
      pMale_output <- pMale/(pNonbinary + pMale + pFemale)
    }
  }
  # All individuals were altered, adjust accordingly
  if (pNonbinary != 0.01 & pMale != 0.49 & pFemale != 0.50){
    pNonbinary_output <- pNonbinary/(pNonbinary + pMale + pFemale)
    pFemale_output <- pFemale/(pNonbinary + pMale + pFemale)
    pMale_output <- pMale/(pNonbinary + pMale + pFemale)
...
# We are using the sample function to randomly select from the unique values present in the Gender column a number of time equivalent to the number of people we want to simulate, and doing so with replacement, so we could select the same value more than once.
# We must add "Nonbinary as a possible value to be sampled because it is not included in the Gender column
df$Gender <- sample(c(unique(df_first$Gender),"Nonbinary"), 
                     nPeople, 
                     replace = TRUE, 
# To better approach reality, the distribution of these Genders are not equal by default.
                 prob = c(pFemale_output, #Probability of generating a female character
                          pMale_output, #Probability of generating a male character
                          pNonbinary_output)) #Probability of generating a nonbinary character

# Now R will randomly sample first and last names for each person based upon the background details simulated in the previous steps. So, for example, the list of possible first names will be constrained by the Gender and culture of the character.
for (i in 1:nPeople){
  df$Surname[i] <- sample(df_sur$Name[df_sur$Background == df$Background[i]], 1)
  if (df$Gender[i] != "Nonbinary")
    df$First[i] <- sample(df_first$Name[df_first$Culture == df$Culture[i] &
                                          df_first$Gender == df$Gender[i]], 1)
  if (df$Gender[i] == "Nonbinary")
    df$First[i] <- sample(df_first$Name[df_first$Culture == df$Culture[i]], 1)
}
# The end product is a dataframe with a number of rows equivalent to nPeople, each representing a simulated person.
return(df)
}